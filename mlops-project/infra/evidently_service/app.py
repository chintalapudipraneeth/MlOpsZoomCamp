...
class MonitoringService:
    ...
    def iterate(self, dataset_name: str, new_rows: pd.DataFrame):
        ...
        if current_size < window_size:
            logging.info(
                "Not enough data for measurement: "
                f"{current_size} of {window_size}. Waiting more data"
            )
            return
...
def configure_service():
    ...
    # it should be generated via the data preparation script
    ...
    for dataset_name, dataset_options in config["datasets"].items():
        ...
        logging.info(
            f"Load reference data for dataset {dataset_name} "
...
def iterate(dataset: str):
    ...
    SERVICE.iterate(
        dataset_name=dataset, new_rows=pd.DataFrame.from_dict(item)
    )
